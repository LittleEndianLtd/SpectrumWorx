...
NT2_TEST_CASE_TPL(svd_scalar, NT2_REAL_TYPES )
{
  ...
  t_t a =  nt2::svd(T(-2));
  NT2_TEST_EQUAL(a, T(2));
  a =  nt2::svd(T(-2), nt2::econ_);
  NT2_TEST_EQUAL(a, T(2));
  a =  nt2::svd(T(-2), 0);
  NT2_TEST_EQUAL(a, T(2));
}
...
NT2_TEST_CASE_TPL(svd1o_ni, NT2_REAL_TYPES )
{
  ...
  for(int k = 0; k < 3; ++k)
  {
    nt2::table<T> a = nt2::reshape(nt2::_(T(1), T(i[k]*j[k])), i[k], j[k]);
    size_t d = std::min(i[k], j[k]);
    a(_(1, d), _(1, d))+= nt2::from_diag(_(T(101), T(100+d)));
    std::cout << "11 a " << i[k] << "X" << j[k] << ", s =  nt2::svd(a);" << std::endl;
    t_t s =  nt2::svd(a);
    t_t a2 = nt2::mtimes(a, nt2::trans(a));
...
NT2_TEST_CASE_TPL(svd3o_ni, NT2_REAL_TYPES )
{
  ...
  for(int k = 0; k < 3; ++k)
  {
    ...
    std::cout << "31 a " << i[k] << "X" << j[k] << ", nt2::tie(u, ds, v)=  nt2::svd(a);" << std::endl; //ok
    nt2::tie(u, ds, v)=  nt2::svd(a);
    NT2_TEST_ULP_EQUAL(a, nt2::mtimes(u, mtimes(ds, trans(v))), 1024);

    std::cout << "32 a " << i[k] << "X" << j[k] << ", nt2::tie(u, ds, v)=  nt2::svd(a, nt2::matrix_);" << std::endl; //ok
    ...
    NT2_TEST_EQUAL(s, diag_of(ds));
    NT2_TEST_ULP_EQUAL(a, nt2::mtimes(u, mtimes(ds, tv)), 1024);

     std::cout << "35 a " << i[k] << "X" << j[k] << ", nt2::tie(u, ds, v)=  nt2::svd(a, nt2::econ_);" << std::endl; //ok
    nt2::tie(u, ds, v)=  nt2::svd(a, nt2::econ_);
    NT2_TEST_ULP_EQUAL(a, nt2::mtimes(u, mtimes(ds, trans(v))), 1024);

    std::cout << "36 a " << i[k] << "X" << j[k] << ", nt2::tie(u, ds, v)=  nt2::svd(a, 0);" << std::endl; //ok
...
NT2_TEST_CASE_TPL(svd1o_ni_sub, NT2_REAL_TYPES )
{
  ...
  nt2::table<T> a = nt2::reshape(nt2::_(T(1), T(16)), 4, 4) + nt2::from_diag(_(T(101), T(104)));
...
NT2_TEST_CASE_TPL(svd3o_ni_sub, NT2_REAL_TYPES )
{
  ...
  NT2_TEST_ULP_EQUAL(a, nt2::mtimes(u(_(1, 2, 8), _), mtimes(ds(_(1, 2, 8), _), trans(v(_(1, 2, 8), _)))), 1024);
