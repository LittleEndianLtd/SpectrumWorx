...
#include "le/parameters/cpp03/factoryMacro.hpp"
#endif // old MSVC
//------------------------------------------------------------------------------
...
#define LE_ADD_INDIVIDUAL_PARAMETER_WORKER( index, parameter )                                                  \
    template <int dummy> struct ParameterAt<index    , dummy> { using type = parameter ; };                     \
    template <int dummy> struct IndexOf    <parameter, dummy> : std::integral_constant<std::uint8_t, index> {}; \
    parameter & get( parameter * ) { return BOOST_PP_CAT( parameter, _ ); }                                     \
