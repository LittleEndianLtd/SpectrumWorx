...
/// -------------------
///
/// Copyright © 2009 - 2016. Little Endian. All rights reserved.
///
////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------
#if defined( _MSC_VER ) && !defined( __clang__ ) && ( _MSC_VER < 1800 )
#include "le/parameters/cpp03/parameter.hpp"
#endif // old MSVC
//------------------------------------------------------------------------------ 
...
public:
    ...
    value_type const & getValue(                  ) const { LE_ASSERT_MSG( this->isValidValue( value_ ), "Parameter in inconsistent state."                               ); return value_;  }
    void               setValue( param_type value )       { LE_ASSERT_MSG( this->isValidValue( value  ), "Specified value is invalid or out of range for this parameter." ); value_ = value; }
...
public:
    ...
    Parameter & operator++()
    {
        LE_ASSERT_MSG( getValue() != ImplTraits::maximum(), "Tried to increment a parameter past its maximum value." );
        ImplTraits::increment( value_ );
        LE_ASSERT_MSG( ImplTraits::isValidValue( value_ ),  "Parameter in inconsistent state after increment."       );
        return *this;
    }

    Parameter & operator--()
    {
        LE_ASSERT_MSG( getValue() != ImplTraits::minimum(), "Tried to decrement a parameter below its minimum value." );
        ImplTraits::decrement( value_ );
        LE_ASSERT_MSG( ImplTraits::isValidValue( value_ ),  "Parameter in inconsistent state after decrement."        );
        return *this;
    }
