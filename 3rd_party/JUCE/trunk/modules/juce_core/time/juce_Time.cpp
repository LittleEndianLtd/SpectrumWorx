...
namespace TimeHelpers
{
    static struct tm millisToLocal (const int64 millis) noexcept
    {
        ...
        {
        #if !defined( LE_PATCHED_JUCE ) || !defined( JUCE_WINDOWS )
            time_t now = static_cast <time_t> (seconds);
                ...
                result.tm_sec   = localTime.wSecond;
                result.tm_min   = localTime.wMinute;
                result.tm_hour  = localTime.wHour;
                result.tm_mday  = localTime.wDay;
                result.tm_mon   = localTime.wMonth - 1;
                result.tm_year  = localTime.wYear - 1900;
                result.tm_wday  = localTime.wDayOfWeek;

                {
                    // http://www.opensource.apple.com/source/ruby/ruby-67/ruby/wince/time_wce.c
                    SYSTEMTIME localYear = { 0 };
                    localYear.wYear      = localTime.wYear;
                    localYear.wMonth     = 1;
                    localYear.wDayOfWeek = 1;
                    localYear.wDay       = 1;
                    unsigned long long localYearns;
                    ::SystemTimeToFileTime( &localYear, reinterpret_cast<FILETIME *>( &localYearns ) );

                    unsigned long long localTimens;
                    ::SystemTimeToFileTime( &localTime, reinterpret_cast<FILETIME *>( &localTimens ) );

                    unsigned long long const yearOffsetns( localTimens - localYearns );
                    unsigned long long const hundredNsPerSec( 10000000 );
                    result.tm_yday = static_cast<unsigned int>( yearOffsetns / hundredNsPerSec ) / ( 60 * 60 * 24 );
                    jassert( result.tm_yday <= 366 );
                }

                {
                    TIME_ZONE_INFORMATION dummy;
                    DWORD const dstInfo( ::GetTimeZoneInformation( &dummy ) );
                    switch ( dstInfo )
                    {
                        case TIME_ZONE_ID_STANDARD: result.tm_isdst = 0; break;
                        case TIME_ZONE_ID_DAYLIGHT: result.tm_isdst = 1; break;
            ...
            const bool useLocalTime) noexcept
{
    ...
    else
    {
        ...
        JUCE_ORIGINAL( millisSinceEpoch = 1000 * (int64) mktime (&t); )
...
int64 Time::currentTimeMillis() noexcept
{
    ...
    /* * Number of 100 nanosecond units from 1/1/1601 to 1/1/1970 */
        unsigned long long const epoch_bias( 116444736000000000ULL );
        unsigned long long utcTime;
        ::GetSystemTimeAsFileTime( reinterpret_cast<FILETIME *>( &utcTime ) );
        return ( utcTime - epoch_bias ) / 10 / 1000;
    #else
    struct _timeb t;
   #ifdef _INC_TIME_INL
    _ftime_s (&t);
