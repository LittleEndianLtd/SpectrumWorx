...
public:
    ...
    WeakReference (ObjectType* const object) LE_PATCH( noexcept )  : holder (getRef (object)) {}
    ...
    */
    ...
    public:
            ...
            Master( ObjectType & object ) : sharedPointer( new SharedPointer( &object ) ) {}
        #else
            Master() noexcept {}
        #endif // LE_PATCHED_JUCE

    #if !defined( NDEBUG ) || !defined( LE_PATCHED_JUCE )
        ~Master()
        {
        ...
        */
        SharedPointer* getSharedPointer (ObjectType* const object) LE_PATCH( noexcept )
        {
        #ifdef LE_PATCHED_JUCE
            jassert( sharedPointer->get() == object );
            (void)object;
        #else
        ...
        */
        void clear() LE_PATCH( noexcept )
        {
        #ifdef LE_PATCHED_JUCE
            LE_PATCH_ASSUME( sharedPointer.getObject() );
        #else
            if (sharedPointer != nullptr)
        #endif // LE_PATCHED_JUCE
