...
public:
    ...
    static CharPointerType createFromCharPointer (const CharPointer text)
    {
        ...
        return createFromCharPointer( text, std::numeric_limits<std::size_t>::max() );
    #else
    ...
    #ifdef LE_PATCHED_JUCE
        static CharPointerType createFromCharPointer( CharPointer_ASCII const text )
        {
            if (text.getAddress() == nullptr || text.isEmpty())
                return getEmpty();

            unsigned int const lengthWithTerminator( static_cast<unsigned int>( text.sizeInBytes() ) );
            unsigned int const bytesNeeded         ( lengthWithTerminator * sizeof( CharType )       );

            const CharPointerType dest (createUninitialisedBytes (bytesNeeded));
            std::copy( text.getAddress(), text.getAddress() + lengthWithTerminator, dest.getAddress() );
            return dest;
        }

        static CharPointerType createFromCharPointer( CharPointer_ASCII const text, std::size_t const maxChars )
        {
            if (text.getAddress() == nullptr || text.isEmpty())
                return getEmpty();

            unsigned int const length     ( static_cast<unsigned int>( text.lengthUpTo( maxChars ) ) );
            unsigned int const bytesNeeded( ( length + 1 ) * sizeof( CharType )                      );

            const CharPointerType dest (createUninitialisedBytes (bytesNeeded));
            *std::copy( text.getAddress(), text.getAddress() + length, dest.getAddress() ) = 0;
            return dest;
        }

        static CharPointerType createFromCharPointer( CharPointerType const text )
        {
            if (text.getAddress() == nullptr || text.isEmpty())
                return getEmpty();

            unsigned int const bytesNeeded( static_cast<unsigned int>( text.sizeInBytes() ) );

            const CharPointerType dest (createUninitialisedBytes (bytesNeeded));
            std::memcpy( dest.getAddress(), text.getAddress(), bytesNeeded );
            return dest;
        }

        static CharPointerType createFromCharPointer( CharPointerType const text, std::size_t const maxChars )
        {
            if (text.getAddress() == nullptr || text.isEmpty())
                return getEmpty();

        #if JUCE_STRING_UTF_TYPE == 8
            unsigned int const destLength ( static_cast<unsigned int>( /*std*/::strnlen( text, maxChars ) ) );
            unsigned int const bytesNeeded( ( destLength + 1 ) * sizeof( CharType )                         );
        #else
            unsigned int const destLength ( static_cast<unsigned int>( text.lengthUpTo( maxChars ) ) );
            unsigned int const bytesNeeded( ( destLength + 1 ) * sizeof( CharType )                  );
        #endif // JUCE_STRING_UTF_TYPE

            const CharPointerType dest (createUninitialisedBytes (bytesNeeded));
            std::memcpy( dest.getAddress(), text.getAddress(), bytesNeeded );
            dest.getAddress()[ destLength ] = 0;
            return dest;
        }
    #endif // LE_PACTHED_JUCE
    ...
    {
    #ifdef LE_PATCHED_JUCE
        return createFromCharPointer( start, end - start );
    #else
...
namespace NumberToStringConverters
{
...
#endif // LE_PATCHED_JUCE
    JUCE_ORIGINAL( static ) char* numberToString (char* t, const int64 n) noexcept LE_PATCH( ; )
#ifndef LE_PATCHED_JUCE
    {
        if (n >= 0)
...
#endif // LE_PATCHED_JUCE
    JUCE_ORIGINAL( static ) char* numberToString (char* t, uint64 v) noexcept LE_PATCH( ; )
#ifndef LE_PATCHED_JUCE
    {
        return printDigits (t, v);
    }
#endif // LE_PATCHED_JUCE
    JUCE_ORIGINAL( static ) char* numberToString (char* t, const int n) noexcept LE_PATCH( ; )
#ifndef LE_PATCHED_JUCE
    {
        if (n >= 0)
...
#endif // LE_PATCHED_JUCE
    JUCE_ORIGINAL( static ) char* numberToString (char* t, unsigned int v) noexcept LE_PATCH( ; )
#ifndef LE_PATCHED_JUCE
    {
        return printDigits (t, v);
    }
...
#endif // LE_PATCHED_JUCE
    JUCE_ORIGINAL( static ) char* doubleToString (char* buffer, const int numChars, double n, int numDecPlaces, size_t& len) noexcept LE_PATCH( ; )
#ifndef LE_PATCHED_JUCE
    {
        JUCE_ORIGINAL( if (numDecPlaces > 0 && n > -1.0e20 && n < 1.0e20) )
    #if !defined( LE_PATCHED_JUCE ) || defined( _MSC_VER )
        {
            char* const end = buffer + numChars;
            char* t = end;
            JUCE_ORIGINAL( int64 v = (int64) ) LE_PATCH( int v = (int) ) (pow (10.0, numDecPlaces) * std::abs (n) + 0.5);
            *--t = (char) 0;
            ...
            len = (size_t) (end - t - 1);
            LE_PATCH( std::memmove( buffer, t, len + 1 ); )
            return t;
        }
    #else // LE_PATCHED_JUCE
    #ifdef LE_PATCHED_JUCE
        static char format[] = "%.9f";
        format[ 2 ] = '0' + static_cast<char>( numDecPlaces );
    ...
    static String::CharPointerType createFromInteger (const IntegerType number)
    {
        ...
        char const * const end  ( numberToString( start, number ) + 1 );
#else
    ...
    #ifdef LE_PATCHED_JUCE
        char * const start( buffer );
        doubleToString( buffer, numElementsInArray (buffer), number, numberOfDecimalPlaces, len);
    #else
...
{
    return t != nullptr ? text.compareIgnoreCase (JUCE_ORIGINAL( CharPointer_UTF8 ) LE_PATCH( CharPointer_ASCII ) (t)) == 0
                        : isEmpty();
}
...
int String::compare (const char* const other) const noexcept       { return text.compare (CharPointer_ASCII (other)); }
#else
...
String& String::operator+= (const int number)
{
    char buffer [16];
#ifdef LE_PATCHED_JUCE
    NumberToStringConverters::numberToString( buffer, number );
    return *this += buffer;
#else
    char* const end = buffer + numElementsInArray (buffer);
...
#if ! JUCE_NATIVE_WCHAR_IS_UTF32
JUCE_ORIGINAL( JUCE_API String JUCE_CALLTYPE operator+ (const juce_wchar s1, const String& s2)     { return String::charToString (s1) + s2; } )
JUCE_ORIGINAL( JUCE_API String JUCE_CALLTYPE operator+ (String s1, const juce_wchar s2)            { return s1 += s2; } )
JUCE_API String& JUCE_CALLTYPE operator<< (String& s1, const juce_wchar s2)         { return s1 += s2; }
#endif

JUCE_API String& JUCE_CALLTYPE operator<< (String& s1, const char s2)               { return s1 += s2; }
JUCE_ORIGINAL( JUCE_API String& JUCE_CALLTYPE operator<< (String& s1, const wchar_t s2)            { return s1 += s2; } )
...
{
#if defined( LE_PATCHED_JUCE ) && defined( _WIN32 ) && ( JUCE_STRING_UTF_TYPE == 16 )
    wchar_t const * const pCharacter( std::wcsrchr( text, character ) );
    return pCharacter ? static_cast<unsigned int>( pCharacter - text.getAddress() ) : -1;
#else
    CharPointerType t (text);
    int last = -1;
...
#ifdef LE_PATCHED_JUCE
bool String::containsAnyOf (char const * const characters) const noexcept
{
    CharPointerType         t    ( text       );
    CharPointer_ASCII const chars( characters );
    while (! t.isEmpty())
        if ( chars.indexOf( t.getAndAdvance() ) >= 0 )
            return true;
