...
void MemoryOutputStream::flush() LE_PATCH( noexcept )
{
    trimExternalBlockSize();
}
...
bool MemoryOutputStream::write (const void* const buffer, size_t howMany) LE_PATCH( noexcept )
{
...
bool MemoryOutputStream::writeRepeatedByte (uint8 byte, size_t howMany) LE_PATCH( noexcept )
{
    if (howMany == 0)
        return true;
...
bool MemoryOutputStream::setPosition (int64 newPosition) LE_PATCH( noexcept )
{
