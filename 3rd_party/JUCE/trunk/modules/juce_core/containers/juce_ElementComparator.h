...
static void sortArray (ElementComparator& comparator,
                       ...
                       LE_PATCH( unsigned ) int firstElement,
                       LE_PATCH( unsigned ) int lastElement,
                       const bool retainOrderOfEquivalentItems)
{
    ...
    if (lastElement > firstElement)
    {
        ...
        {
            for (LE_PATCH( unsigned ) int i = firstElement; i < lastElement; ++i)
            {
        ...
        {
            LE_PATCH( unsigned ) int fromStack[30], toStack[30];
            int stackIndex = 0;

            for (;;)
            {
                const LE_PATCH( unsigned ) int size = (lastElement - firstElement) + 1;

                if (size <= 8)
                {
                    LE_PATCH( unsigned ) int j = lastElement;
                    LE_PATCH( unsigned ) int maxIndex;

                    while (j > firstElement)
                    {
                        maxIndex = firstElement;
                        for (LE_PATCH( unsigned ) int k = firstElement + 1; k <= j; ++k)
                ...
                {
                    const LE_PATCH( unsigned ) int mid = firstElement + (size >> 1);
                    std::swap (array[mid], array[firstElement]);

                    LE_PATCH( unsigned ) int i = firstElement;
                    LE_PATCH( unsigned ) int j = lastElement + 1;
                ...
                jassert (LE_PATCH( (unsigned) )stackIndex < numElementsInArray (fromStack));
