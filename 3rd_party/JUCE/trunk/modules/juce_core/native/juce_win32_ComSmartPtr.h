...
inline GUID uuidFromString (const char* const s) noexcept
{
    ...
    sscanf
  #else
    LE_PATCH( sscanf ) JUCE_ORIGINAL( sscanf_s )
  #endif
...
//==============================================================================
template <class ComClass>
class LE_PATCH_NOVTABLE ComBaseClassHelperBase   : public ComClass
{
public:
    ComBaseClassHelperBase (unsigned int initialRefCount)  : refCount (initialRefCount) {}
    JUCE_ORIGINAL( virtual ) ~ComBaseClassHelperBase() LE_PATCH( noexcept ) {}
...
protected:
    ...
    {
        this->AddRef(); *result = JUCE_ORIGINAL( dynamic_cast ) LE_PATCH( static_cast ) <Type*> (this); return S_OK;
    }
};
