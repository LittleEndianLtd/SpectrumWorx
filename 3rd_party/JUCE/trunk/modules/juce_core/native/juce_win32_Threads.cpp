...
LE_PATCH( LE_WEAK_SYMBOL ) JUCE_ORIGINAL( static ) SleepEvent sleepEvent;

void JUCE_CALLTYPE Thread::sleep (const int millisecs)
{
    if (millisecs >= 10 JUCE_ORIGINAL( || sleepEvent.handle == 0 ))
    {
    ...
    else
    {
        ...
        DWORD const result( WaitForSingleObject (sleepEvent.handle, millisecs) );
        jassert( result == WAIT_TIMEOUT ); result;
    #else
...
{
    LE_PATCH_ASSUME( currentModuleHandle );
...
#ifdef LE_PATCHED_JUCE
bool juce_isRunningInWine() { return false; }
#else
