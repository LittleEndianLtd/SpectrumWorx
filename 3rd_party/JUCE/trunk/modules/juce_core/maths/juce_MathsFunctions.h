...
inline Type jmin (const Type a, const Type b, const Type c, const Type d)                   { return jmin (a, jmin (b, c, d)); }
    ...
    float jmin( float const & value1, float const & value2 );
    float jmax( float const & value1, float const & value2 );
...
inline LE_PATCH( unsigned ) int numElementsInArray (Type (&array)[N])
{
...
#ifdef LE_PATCHED_JUCE
inline float juce_hypot( float const & a, float const & b ) noexcept
{
#if JUCE_WINDOWS
    return _hypotf( a, b );
#else
...
#ifdef LE_PATCHED_JUCE
    #ifdef _MSC_VER
        #define LE_JUCE_FORCEINLINE __forceinline
    #else
        #define LE_JUCE_FORCEINLINE __attribute__((always_inline)) inline
    #endif
    LE_JUCE_FORCEINLINE int roundToInt( float  const value ) { return _mm_cvtss_si32( _mm_set_ss (  value ) ); }
    LE_JUCE_FORCEINLINE int roundToInt( double const value ) { return _mm_cvtsd_si32( _mm_load_sd( &value ) ); }
    template <typename IntegralType>
    inline IntegralType const & roundToInt( IntegralType const & value ) { return value; }
    #undef LE_JUCE_FORCEINLINE
#else
