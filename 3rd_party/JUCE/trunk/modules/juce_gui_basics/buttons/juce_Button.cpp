...
Button::Button (const String& name)
  : Component (name),
    ...
    buttonPressTime (0),
    lastRepeatTime (0),
    JUCE_ORIGINAL( commandManagerToUse (nullptr), )
    autoRepeatDelay (-1),
    autoRepeatSpeed (0),
...
Button::~Button() LE_PATCH( noexcept )
{
...
//==============================================================================
void Button::setButtonText (const String& newText) LE_PATCH( noexcept )
{
    if (text != newText)
    {
        text = newText;
...
#ifdef LE_PATCHED_JUCE
bool Button::getToggleState() const noexcept
{
    jassert( isOn.getValue().isBool() );
    struct LE_PATCH_NOVTABLE SimpleValueSource : Value::ValueSource { var value; };
    bool const & result( static_cast<SimpleValueSource const &>( const_cast<Value &>( isOn ).getValueSource() ).value.value.boolValue );
    jassert( result == static_cast<bool>( isOn.getValue() ) );
    return result;
}
#endif // LE_PATCHED_JUCE
...
void Button::valueChanged (Value& value) LE_PATCH( noexcept )
{
...
void Button::clicked (const ModifierKeys&) LE_PATCH( noexcept )
{
    clicked();
}
...
void Button::mouseExit (const MouseEvent&) LE_PATCH( noexcept )
{
...
void Button::mouseUp (const MouseEvent& e) LE_PATCH( noexcept )
{
...
void Button::mouseDrag (const MouseEvent&) LE_PATCH( noexcept )
{
...
#ifdef LE_PATCHED_JUCE
    LE_PATCH_ASSUME( !commandManagerToUse_ );
#else
...
bool Button::keyStateChanged (const bool, Component*) LE_PATCH( noexcept )
{
    if (! isEnabled())
        return false;
