...
public:
    ...
    */
    MouseEvent (MouseInputSource& source,
                ...
                Time LE_PATCH( const & ) eventTime,
                Point<int> LE_PATCH( const & ) mouseDownPos,
                Time LE_PATCH( const & ) mouseDownTime,
                int numberOfClicks,
                bool mouseWasDragged) noexcept;

    /** Destructor. */
    JUCE_ORIGINAL( ~MouseEvent() noexcept; )
    ...
    */
    MouseEvent withNewPosition (Point<int> LE_PATCH( const & ) newPosition) const noexcept;
