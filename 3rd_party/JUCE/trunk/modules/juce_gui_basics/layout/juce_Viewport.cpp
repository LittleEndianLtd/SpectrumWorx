...
Point<int> Viewport::viewportPosToCompPos (Point<int> LE_PATCH( const & ) pos) const
{
...
void Viewport::setViewPosition (Point<int> LE_PATCH( const & ) newPosition)
{
    if (contentComp != nullptr)
...
void Viewport::componentMovedOrResized (Component&, bool, bool) LE_PATCH( noexcept )
{
    updateVisibleArea();
}
...
void Viewport::mouseWheelMove (const MouseEvent& e, const MouseWheelDetails& wheel) LE_PATCH( noexcept )
{
