...
LE_PATCH( public: )
    ...
    */
    ...
    public:
        /** Destructor. */
        virtual ~Listener() LE_PATCH( noexcept ) {}

        /** Called when something in the given marker list changes. */
    #ifndef LE_PATCHED_JUCE
        virtual void markersChanged (MarkerList* markerList) LE_PATCH( noexcept ) = 0;

        /** Called when the given marker list is being deleted. */
        virtual void markerListBeingDeleted (MarkerList* markerList) LE_PATCH( noexcept );
    #endif // LE_PATCHED_JUCE
    };
