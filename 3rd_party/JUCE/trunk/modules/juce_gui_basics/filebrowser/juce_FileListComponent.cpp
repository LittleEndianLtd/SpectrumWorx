...
public:
                 ...
                 const bool highlighted_)
    {
        ...
        if (fileInfo != nullptr)
        {
            ...
            if ( fileInfo->filename != JUCE_T("..") )
            {
        #endif // LE_PATCHED_JUCE
            newFile = root.getChildFile (fileInfo->filename);
            JUCE_ORIGINAL( newFileSize = File::descriptionOfSizeInBytes (fileInfo->fileSize); )
            JUCE_ORIGINAL( newModTime = fileInfo->modificationTime.formatted ("%d %b '%y %H:%M"); )
        #ifdef LE_PATCHED_JUCE
            }
            else
            {
                newFile = File::createFileWithoutCheckingPath
                (
...
//==============================================================================
int FileListComponent::getNumRows() LE_PATCH( noexcept )
{
...
void FileListComponent::deleteKeyPressed (int /*currentSelectedRow*/) LE_PATCH( noexcept )
{
}
