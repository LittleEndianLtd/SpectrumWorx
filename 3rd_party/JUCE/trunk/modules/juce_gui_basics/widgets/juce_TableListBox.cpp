...
int TableListBox::getNumRows() LE_PATCH( noexcept )
{
...
void TableListBox::deleteKeyPressed (int row) LE_PATCH( noexcept )
{
    if (model != nullptr)
...
void TableListBox::backgroundClicked() LE_PATCH( noexcept )
{
    if (model != nullptr)
        model->backgroundClicked();
}

void TableListBox::listWasScrolled() LE_PATCH( noexcept )
{
    if (model != nullptr)
