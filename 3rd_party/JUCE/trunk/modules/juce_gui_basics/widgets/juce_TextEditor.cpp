...
TextEditor::~TextEditor() LE_PATCH( noexcept )
{
    if (wasFocused)
...
void TextEditor::mouseWheelMove (const MouseEvent& e, const MouseWheelDetails& wheel) LE_PATCH( noexcept )
{
...
bool TextEditor::keyStateChanged (const bool isKeyDown) LE_PATCH( noexcept )
{
    if (! isKeyDown)
        return false;
