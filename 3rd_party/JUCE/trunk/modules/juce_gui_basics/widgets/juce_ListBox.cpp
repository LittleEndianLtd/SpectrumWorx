...
public:
    ...
    void mouseDrag (const MouseEvent& e) LE_PATCH( noexcept ) override
    {
    #ifdef LE_PATCHED_JUCE
        LE_PATCH_ASSUME( !isDragging ); (void)e;
    #else
        if (ListBoxModel* m = owner.getModel())
        {
...
public:
    ...
    void mouseMove (const MouseEvent& e) LE_PATCH( noexcept ) override
    {
        const MouseEvent e2 (e.getEventRelativeTo (&owner));
        owner.selectRow (owner.getRowContainingPosition (e2.x, e2.y), true);
    }

    void mouseExit (const MouseEvent& e) LE_PATCH( noexcept ) override
    {
        mouseMove (e);
    }
...
{
#ifdef LE_PATCHED_JUCE
    LE_PATCH_ASSUME( !multipleSelection );
#else
...
bool ListBox::keyStateChanged (const bool isKeyDown) LE_PATCH( noexcept )
{
    return isKeyDown
...
void ListBox::mouseWheelMove (const MouseEvent& e, const MouseWheelDetails& wheel) LE_PATCH( noexcept )
{
...
void ListBox::mouseUp (const MouseEvent& e) LE_PATCH( noexcept )
{
...
void ListBoxModel::listBoxItemClicked (int, const MouseEvent&) LE_PATCH( noexcept ) {}
void ListBoxModel::listBoxItemDoubleClicked (int, const MouseEvent&) {}
void ListBoxModel::backgroundClicked() LE_PATCH( noexcept ) {}
void ListBoxModel::selectedRowsChanged (int) {}
void ListBoxModel::deleteKeyPressed (int) LE_PATCH( noexcept ) {}
void ListBoxModel::returnKeyPressed (int) {}
void ListBoxModel::listWasScrolled() LE_PATCH( noexcept ) {}
