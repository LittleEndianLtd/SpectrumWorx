...
inline BOOL updateLayeredWindow( HWND hwnd, HDC hdcDst, POINT * pptDst, SIZE * psize, HDC hdcSrc, POINT * pptSrc, COLORREF crKey, BLENDFUNCTION * pblend, DWORD dwFlags )
{
...
int64 getMouseEventTime() LE_PATCH( noexcept )
{
...
public:
    ...
    void initialiseBitmapData (Image::BitmapData& bitmap, int x, int y, Image::BitmapData::ReadWriteMode) LE_PATCH( noexcept ) override
    {
...
class HostileKeyboardHooker
{
private:
    class FocusChangeHandler sealed : public FocusChangeListener
    {
        void globalFocusChanged( Component * pFocusedComponent );
    };

protected:
     HostileKeyboardHooker();
    ~HostileKeyboardHooker();

    static void peerGainedFocus( HWNDComponentPeer & );
    static void peerLostFocus  ( HWNDComponentPeer & );
...
private:
    ...
    static LRESULT CALLBACK wrapperWindowProc    ( HWND h, UINT message, WPARAM wParam, LPARAM lParam );

private:
    static HWNDComponentPeer  * pActiveHWNDComponentPeer_;
    static FocusChangeHandler   focusChangeHandler_;
    static HHOOK                keyboardHookHandle_;
    static unsigned int         referenceCount_;
}; // HostileKeyboardHooker
#endif // LE_PATCHED_JUCE


//==============================================================================
class HWNDComponentPeer LE_PATCH( sealed ) : public ComponentPeer LE_PATCH( , private HostileKeyboardHooker )
{
public:
    ...
    HWNDComponentPeer (Component& comp, const int windowStyleFlags, HWND parent, bool nonRepainting)
        : ComponentPeer (comp, windowStyleFlags),
          ...
          currentWindowIcon (0),
          JUCE_ORIGINAL( dropTarget (nullptr), )
          updateLayeredWindowAlpha (255)
    {
        ...
        {
        #ifdef LE_PATCHED_JUCE
            //...mrmlj...AlertWindow...LE_PATCH_UNREACHABLE_CODE
        #else
    ...
    ~HWNDComponentPeer()
    {
        JUCE_ORIGINAL( shadower = nullptr; )
    ...
    //==============================================================================
    void* getNativeHandle() const LE_PATCH( noexcept ) override    { return hwnd; }

    void setVisible (bool shouldBeVisible) LE_PATCH( noexcept ) override
    {
    ...
    void setTitle (const String& title) LE_PATCH( noexcept ) override
    {
    ...
    void setBounds (const Rectangle<int>& bounds, bool isNowFullScreen) LE_PATCH( noexcept ) override
    {
    ...
    Rectangle<int> getBounds() const LE_PATCH( noexcept ) override
    {
    ...
    Point<int> getScreenPosition() const LE_PATCH( noexcept )
    {
    ...
    Point<int> localToGlobal (Point<int> LE_PATCH( const & ) relativePosition) LE_PATCH( noexcept ) override  { return relativePosition + getScreenPosition(); }
    Point<int> globalToLocal (Point<int> LE_PATCH( const & ) screenPosition) LE_PATCH( noexcept ) override    { return screenPosition   - getScreenPosition(); }

    void setAlpha (float newAlpha) LE_PATCH( noexcept ) override
    {
    ...
    void setMinimised (bool shouldBeMinimised) LE_PATCH( noexcept ) override
    {
        if (shouldBeMinimised != isMinimised())
            ShowWindow (hwnd, shouldBeMinimised ? SW_MINIMIZE : SW_SHOWNORMAL);
    }

    bool isMinimised() const LE_PATCH( noexcept ) override
    {
        WINDOWPLACEMENT wp;
    ...
    void setFullScreen (bool shouldBeFullScreen) LE_PATCH( noexcept ) override
    {
        setMinimised (false);
    ...
    bool isFullScreen() const LE_PATCH( noexcept ) override
    {
        if (! hasTitleBar())
            return fullScreen;
    ...
    bool contains (Point<int> LE_PATCH( const & ) localPos, bool trueIfInAChildWindow) const LE_PATCH( noexcept ) override
    {
        RECT r = getWindowRect (hwnd);
    ...
    BorderSize<int> getFrameSize() const LE_PATCH( noexcept ) override
    {
        return windowBorder;
    }

    bool setAlwaysOnTop (bool alwaysOnTop) LE_PATCH( noexcept ) override
    {
    ...
    void toFront (bool makeActive) LE_PATCH( noexcept ) override
    {
        setMinimised (false);
    ...
    void toBehind (ComponentPeer* other) LE_PATCH( noexcept ) override
    {
        if (HWNDComponentPeer* const otherPeer = JUCE_ORIGINAL( dynamic_cast ) LE_PATCH( static_cast ) <HWNDComponentPeer*> (other))
        {
            setMinimised (false);
    ...
    bool isFocused() const LE_PATCH( noexcept ) override
    {
        return callFunctionIfNotLocked (&getFocusCallback, 0) == (void*) hwnd;
    }

    void grabFocus() LE_PATCH( noexcept ) override
    {
        const bool oldDeactivate = shouldDeactivateTitleBar;
        shouldDeactivateTitleBar = ((styleFlags & windowIsTemporary) == 0);
    ...
    void textInputRequired (const Point<int>&) LE_PATCH( noexcept ) override
    {
        if (! hasCreatedCaret)
        {
    ...
    void dismissPendingTextInput() LE_PATCH( noexcept ) override
    {
        JUCE_ORIGINAL( imeHandler.handleSetContext (hwnd, false); )
    }

    void repaint (const Rectangle<int>& area) LE_PATCH( noexcept ) override
    {
        const RECT r = { area.getX(), area.getY(), area.getRight(), area.getBottom() };
        InvalidateRect (hwnd, &r, FALSE);
    }

    void performAnyPendingRepaintsNow() LE_PATCH( noexcept ) override
    {
        MSG m;
    ...
    static void updateModifiersFromWParam (const WPARAM wParam) LE_PATCH( noexcept )
    {
        int mouseMods = 0;
...
private:
    ...
    JUCE_ORIGINAL( JuceDropTarget* dropTarget; )
    uint8 updateLayeredWindowAlpha;
    JUCE_ORIGINAL( MultiTouchMapper<DWORD> currentTouches; )
    ...
    void createWindow()
    {
        ...
        {
            // LE PATCH: we want owned windows
            JUCE_ORIGINAL( type |= WS_CHILD; )
        }
        else
        {
    ...
    {
        JUCE_ORIGINAL( RevokeDragDrop ((HWND) handle); )
    ...
    int getCurrentRenderingEngine() const LE_PATCH( noexcept ) override   { return JUCE_ORIGINAL( currentRenderingEngine ) LE_PATCH( softwareRenderingEngine ); }
    ...
    void setCurrentRenderingEngine (int index) LE_PATCH( noexcept ) override
    {
        (void) index;
    ...
    LE_PATCH( friend class HostileKeyboardHooker; )
...
private:
    ...
    LRESULT peerWindowProc (HWND h, UINT message, WPARAM wParam, LPARAM lParam)
    {
        switch (message)
        {
            ...
            //==============================================================================
            case WM_SETFOCUS:
                HostileKeyboardHooker::peerGainedFocus( *this );
                updateKeyModifiers();
                handleFocusGain();
                break;
            ...
            case WM_KILLFOCUS:
                ...
                HostileKeyboardHooker::peerLostFocus( *this );
                handleFocusLoss();
                break;
...
#ifdef LE_PATCHED_JUCE
    // ...we disable IME for now...
    // http://rawmaterialsoftware.com/viewtopic.php?f=3&t=6899&hilit=IME
...
HWNDComponentPeer                         * HostileKeyboardHooker::pActiveHWNDComponentPeer_( 0 );
HostileKeyboardHooker::FocusChangeHandler   HostileKeyboardHooker::focusChangeHandler_           ;
HHOOK                                       HostileKeyboardHooker::keyboardHookHandle_      ( 0 );
unsigned int                                HostileKeyboardHooker::referenceCount_          ( 0 );


HostileKeyboardHooker::HostileKeyboardHooker()
{
    if ( referenceCount_++ == 0 )
    {
        jassert( !keyboardHookHandle_ );
        jassert( !pActiveHWNDComponentPeer_ );
        Desktop::getInstance().addFocusChangeListener( &focusChangeHandler_ );
    }
}
...
HostileKeyboardHooker::~HostileKeyboardHooker()
{
    if ( --referenceCount_ == 0 )
    {
        jassert( !pActiveHWNDComponentPeer_ || ( pActiveHWNDComponentPeer_ == &this->peer() ) );
        unhook();
        Desktop::getInstance().removeFocusChangeListener( &focusChangeHandler_ );
    }
    peerLostFocus( this->peer() );
}


LRESULT CALLBACK HostileKeyboardHooker::keyboardHookProcedure( int const code, WPARAM const wParam, LPARAM const lParam )
{
    // Implementation note:
    //   We grab/'prehandle' messages only when they are actually removed from
    // the message queue and when they are not intended for a modal component
    // (these have their own message loops).
    //                                        (19.03.2010.) (Domagoj Saric)
    if
    (
        ( code == HC_ACTION ) &&
        !pActiveHWNDComponentPeer_->getComponent().isCurrentlyModal()
    )
    {
    #ifndef NDEBUG
        Component * const pFocusedComponent( Component::getCurrentlyFocusedComponent() );
        jassert( pFocusedComponent );
        ComponentPeer * const pPeer( pFocusedComponent->getPeer() );
        jassert( pPeer );
        jassert( dynamic_cast<HWNDComponentPeer *>( pPeer ) );
        jassert( pActiveHWNDComponentPeer_ );
        jassert( pPeer == pActiveHWNDComponentPeer_ );
    #endif // NDEBUG
        UINT const keyMessage
        (
            #ifdef _M_X64
                ::_bittest64( &lParam, 31 )
            #else
                ::_bittest  ( &lParam, 31 )
            #endif // _M_X64
                ? WM_KEYUP
        ...
        bool const consumed
        (
            ( keyMessage == WM_KEYDOWN )
                ? pActiveHWNDComponentPeer_->doKeyDown( wParam )
                : pActiveHWNDComponentPeer_->doKeyUp  ( wParam )
        );
        if ( consumed )
        {
            MSG msg;
            msg.hwnd    = static_cast<HWND>( pActiveHWNDComponentPeer_->HWNDComponentPeer::getNativeHandle() );
            msg.message = keyMessage;
            msg.wParam  = wParam;
            msg.lParam  = lParam;
            msg.time = 0;
            msg.pt.x = 0;
    ...
    return ::CallNextHookEx( HostileKeyboardHooker::keyboardHookHandle_, code, wParam, lParam );
}


LRESULT CALLBACK HostileKeyboardHooker::wrapperWindowProc
(
...
)
{
    ...
    return alreadyHandled
            ? 0
            : HWNDComponentPeer::windowProc( windowHandle, message, wParam, lParam );
}

#ifndef NDEBUG
extern "C" IMAGE_DOS_HEADER __ImageBase;

static IMAGE_NT_HEADERS const & ntHeader( reinterpret_cast<IMAGE_NT_HEADERS const &>( reinterpret_cast<char const *>( &__ImageBase )[ __ImageBase.e_lfanew ] ) );
static DWORD const binarySize( ntHeader.OptionalHeader.SizeOfImage );
static void const * const moduleStartAddress( &__ImageBase                                                 );
static void const * const moduleEndAddress  ( static_cast<char const *>( moduleStartAddress ) + binarySize );
#endif // NDEBUG

void HostileKeyboardHooker::hook()
{
    jassert( pActiveHWNDComponentPeer_ );
#ifndef NDEBUG
    jassert( Process::getCurrentModuleInstanceHandle() == moduleStartAddress );
#endif // NDEBUG

    if ( !keyboardHookHandle_ )
    {
        keyboardHookHandle_ =
            ::SetWindowsHookEx
            (
                WH_KEYBOARD,
                &HostileKeyboardHooker::keyboardHookProcedure,
                0,
                ::GetCurrentThreadId()
            );
        jassert( keyboardHookHandle_ ); // proper error handling desired...

        LONG_PTR const oldProc
        (
            ::SetWindowLongPtr
            (
                reinterpret_cast<HWND>( pActiveHWNDComponentPeer_->HWNDComponentPeer::getNativeHandle() ),
                GWLP_WNDPROC,
                reinterpret_cast<LONG_PTR>( &HostileKeyboardHooker::wrapperWindowProc )
            )
        );
    #ifndef NDEBUG
        jassert
        (
            ( oldProc == reinterpret_cast<LONG_PTR>( &HWNDComponentPeer::windowProc ) ) ||
            ( // wnd.proc. outside our module (host already hooked it)...
                ( oldProc < reinterpret_cast<LONG_PTR>( moduleStartAddress ) ) ||
                ( oldProc > reinterpret_cast<LONG_PTR>( moduleEndAddress   ) )
            )
        );
    #endif // NDEBUG
        UNREFERENCED_PARAMETER( oldProc );
    }
    else
    {
        // If we switched/'focus changed' peers, not just components, this
        // assertion need not hold.
        //assert( pActiveHWNDComponentPeer_->getLastFocusedSubcomponent()->getWantsKeyboardFocus() );
    }
}


void HostileKeyboardHooker::unhook()
{
    if ( !keyboardHookHandle_ )
    {
        #ifndef NDEBUG
            HWND const handle( pActiveHWNDComponentPeer_ ? reinterpret_cast<HWND>( pActiveHWNDComponentPeer_->getNativeHandle() ) : 0 );
            typedef LRESULT (CALLBACK *WindowProc)( HWND, UINT, WPARAM, LPARAM );
            WindowProc const currentWindowProc
            (
                reinterpret_cast<WindowProc>( ::GetWindowLongPtr( handle, GWLP_WNDPROC ) )
            );
            WindowProc const defaultProc( &HWNDComponentPeer    ::windowProc        );
            WindowProc const hookerProc ( &HostileKeyboardHooker::wrapperWindowProc );
            bool const currentProcIsAHandle( HIWORD( currentWindowProc ) == 0xFFFF );
            bool const windowClosed        ( !::IsWindow( handle )                 );

            jassert( currentWindowProc != hookerProc );
            jassert
            (
                !pActiveHWNDComponentPeer_                                                     ||
                windowClosed                                                                   || // window closed
                currentProcIsAHandle                                                           ||
                ( currentWindowProc == defaultProc )                                           ||
                ( // wnd.proc. outside our module (host already hooked it)...
                    ( currentWindowProc < static_cast<WindowProc>( moduleStartAddress ) ) ||
                    ( currentWindowProc > static_cast<WindowProc>( moduleEndAddress   ) )
                )
            );
        #endif // NDEBUG
        return;
    ...
    (
        ...
        (
            reinterpret_cast<HWND>( pActiveHWNDComponentPeer_->HWNDComponentPeer::getNativeHandle() ),
            GWLP_WNDPROC,
            reinterpret_cast<LONG_PTR>( &HWNDComponentPeer::windowProc )
        )
    );
    jassert
    (
        ( !::IsWindow( reinterpret_cast<HWND>( pActiveHWNDComponentPeer_->getNativeHandle() ) ) && oldProc == 0 ) ||
        ( oldProc == reinterpret_cast<LONG_PTR>( &HostileKeyboardHooker::wrapperWindowProc ) )
    );
    UNREFERENCED_PARAMETER( oldProc );

    BOOL const result( ::UnhookWindowsHookEx( keyboardHookHandle_ ) );
    jassert( result );
    UNREFERENCED_PARAMETER( result );
    keyboardHookHandle_ = 0;
}


void HostileKeyboardHooker::peerGainedFocus( HWNDComponentPeer & focusedPeer )
{
    if ( pActiveHWNDComponentPeer_ && pActiveHWNDComponentPeer_ != &focusedPeer )
        unhook();
    pActiveHWNDComponentPeer_ = &focusedPeer;
}


void HostileKeyboardHooker::peerLostFocus( HWNDComponentPeer & unfocusedPeer )
{
    //   Check if Windows sent WM_SETFOCUS to another peer before sending
    // WM_KILLFOCUS to this one (don't know if Microsoft makes any guarantees
    // about this).
    if ( pActiveHWNDComponentPeer_ == &unfocusedPeer )
    {
        unhook();
        pActiveHWNDComponentPeer_ = 0;
    }
}


HWNDComponentPeer & HostileKeyboardHooker::peer()
{
    return static_cast<HWNDComponentPeer &>( *this );
}


void HostileKeyboardHooker::FocusChangeHandler::globalFocusChanged( Component * const pFocusedComponent )
{
    if ( pFocusedComponent )
    {
        jassert
        (
            pFocusedComponent->getWantsKeyboardFocus() || !pFocusedComponent->isEnabled() ||
            // LFODisplay can temporarily disable focus grabbing for the
            // SpectrumWorxEditor...
            pFocusedComponent == &pActiveHWNDComponentPeer_->getComponent()
        );
        jassert( pFocusedComponent->getPeer() == pActiveHWNDComponentPeer_ );
        HostileKeyboardHooker::hook();
    }
    else
        HostileKeyboardHooker::unhook();
}
...
bool KeyPress::isKeyCurrentlyDown (const int keyCode)
{
    ...
    for (LE_PATCH( unsigned ) int i = 0; i < numElementsInArray (translatedValues); i += 2)
...
{
#ifdef LE_PATCHED_JUCE
    // LE PATCH: we don't need to disable the screen saver
    jassert( isEnabled ); isEnabled;
#else
    if (isEnabled)
