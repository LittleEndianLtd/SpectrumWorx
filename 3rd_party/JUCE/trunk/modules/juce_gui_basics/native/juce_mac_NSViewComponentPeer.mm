...
public:
    ...
    //==============================================================================
    void* getNativeHandle() const LE_PATCH( noexcept ) override    { return view; }
    ...
    bool setDocumentEditedStatus (bool edited) LE_PATCH( noexcept ) override
    {
    ...
    void setBounds (const Rectangle<int>& newBounds, bool isNowFullScreen) LE_PATCH( noexcept ) override
    {
    ...
    Rectangle<int> getBounds (const bool global) const LE_PATCH( noexcept )
    {
        NSRect r = [view frame];
    ...
    Point<int> localToGlobal (Point<int> LE_PATCH( const & ) relativePosition) LE_PATCH( noexcept ) override
    {
        return relativePosition + getBounds (true).getPosition();
    }

    Point<int> globalToLocal (Point<int> LE_PATCH( const & ) screenPosition) LE_PATCH( noexcept ) override
    {
    ...
    void updateFullscreenStatus() LE_PATCH( noexcept )
    {
        if (hasNativeTitleBar())
        {
    ...
    bool hasNativeTitleBar() const LE_PATCH( noexcept )
    {
    ...
    void toFront (bool makeActiveWindow) LE_PATCH( noexcept ) override
    {
        if (isSharedWindow)
    ...
    void toBehind (ComponentPeer* other) LE_PATCH( noexcept ) override
    {
        ...
        jassert (dynamic_cast <NSViewComponentPeer*> (other));
        NSViewComponentPeer* const otherPeer = static_cast <NSViewComponentPeer*> (other);
#else
    ...
    int getCurrentRenderingEngine() const LE_PATCH( noexcept ) override
    {
        LE_PATCH_ASSUME( usingCoreGraphics );
