...
public:
    ...
    */
    ...
    /** Destructor. */
    virtual ~ComponentPeer() LE_PATCH( noexcept );
    ...
    */
    virtual void* getNativeHandle() const LE_PATCH( noexcept ) = 0;

    /** Shows or hides the window. */
    virtual void setVisible (bool shouldBeVisible) LE_PATCH( noexcept ) = 0;

    /** Changes the title of the window. */
    virtual void setTitle (const String& title) LE_PATCH( noexcept ) = 0;
    ...
    */
    virtual bool setDocumentEditedStatus (bool edited) LE_PATCH( noexcept );
    ...
    */
    virtual void setBounds (const Rectangle<int>& newBounds, bool isNowFullScreen) LE_PATCH( noexcept ) = 0;
    ...
    /** Returns the current position and size of the window.
        ...
        relative to the parent window's origin, not the screen origin.
    */
    virtual Rectangle<int> getBounds() const LE_PATCH( noexcept ) = 0;

    /** Converts a position relative to the top-left of this component to screen coordinates. */
    virtual Point<int> localToGlobal (Point<int> LE_PATCH( const & ) relativePosition) LE_PATCH( noexcept ) = 0;

    /** Converts a rectangle relative to the top-left of this component to screen coordinates. */
    virtual Rectangle<int> localToGlobal (const Rectangle<int>& relativePosition) LE_PATCH( noexcept );

    /** Converts a screen coordinate to a position relative to the top-left of this component. */
    virtual Point<int> globalToLocal (Point<int> LE_PATCH( const & ) screenPosition) LE_PATCH( noexcept ) = 0;

    /** Converts a screen area to a position relative to the top-left of this component. */
    virtual Rectangle<int> globalToLocal (const Rectangle<int>& screenPosition) LE_PATCH( noexcept );
    ...
    */
    ...
    virtual void setMinimised (bool shouldBeMinimised) LE_PATCH( noexcept ) = 0;

    /** True if the window is currently minimised. */
    virtual bool isMinimised() const LE_PATCH( noexcept ) = 0;

    /** Enable/disable fullscreen mode for the window. */
    virtual void setFullScreen (bool shouldBeFullScreen) LE_PATCH( noexcept ) = 0;

    /** True if the window is currently full-screen. */
    virtual bool isFullScreen() const LE_PATCH( noexcept ) = 0;
    ...
    */
    virtual bool contains (Point<int> LE_PATCH( const & ) localPos, bool trueIfInAChildWindow) const LE_PATCH( noexcept ) = 0;
    ...
    */
    virtual BorderSize<int> getFrameSize() const LE_PATCH( noexcept ) = 0;
    ...
    */
    ...
    /** This is called if the screen resolution changes.
        ...
        screen size changes.
    */
    virtual void handleScreenSizeChange() LE_PATCH( noexcept );
    ...
    */
    virtual bool setAlwaysOnTop (bool alwaysOnTop) LE_PATCH( noexcept ) = 0;

    /** Brings the window to the top, optionally also giving it focus. */
    virtual void toFront (bool makeActive) LE_PATCH( noexcept ) = 0;

    /** Moves the window to be just behind another one. */
    virtual void toBehind (ComponentPeer* other) LE_PATCH( noexcept ) = 0;
    ...
    */
    ...
    virtual bool isFocused() const LE_PATCH( noexcept ) = 0;

    /** Tries to give the window keyboard focus. */
    virtual void grabFocus() LE_PATCH( noexcept ) = 0;
    ...
    */
    ...
    /** Tells the window that text input may be required at the given position.
        ...
        on the OS.
    */
    virtual void textInputRequired (const Point<int>& position) LE_PATCH( noexcept ) = 0;

    /** If there's some kind of OS input-method in progress, this should dismiss it. */
    virtual void dismissPendingTextInput() LE_PATCH( noexcept );
    ...
    virtual void repaint (const Rectangle<int>& area) LE_PATCH( noexcept ) = 0;
    ...
    */
    virtual void performAnyPendingRepaintsNow() LE_PATCH( noexcept ) = 0;

    /** Changes the window's transparency. */
    virtual void setAlpha (float newAlpha) LE_PATCH( noexcept ) = 0;

    //==============================================================================
    void handleMouseEvent (int touchIndex, const Point<int> LE_PATCH( & ) positionWithinPeer, const ModifierKeys newMods, int64 time);
    void handleMouseWheel (int touchIndex, const Point<int> LE_PATCH( & ) positionWithinPeer, int64 time, const MouseWheelDetails&);
    void handleMagnifyGesture (int touchIndex, const Point<int> LE_PATCH( & ) positionWithinPeer, int64 time, float scaleFactor);
    ...
    */
    ...
    virtual int getCurrentRenderingEngine() const LE_PATCH( noexcept );
    virtual void setCurrentRenderingEngine (int index) LE_PATCH( noexcept );
