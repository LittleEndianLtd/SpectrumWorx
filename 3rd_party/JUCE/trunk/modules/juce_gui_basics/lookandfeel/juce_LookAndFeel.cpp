...
{
    static Colour createBaseColour (const Colour JUCE_ORIGINAL( & ) buttonColour,
    ...
    static TextLayout layoutTooltipText (const String& text, const Colour JUCE_ORIGINAL( & ) colour) noexcept
    {
    ...
    : useNativeAlertWindows (false)
{
    ...
    */
    ...
    for (LE_PATCH( unsigned ) int i = 0; i < numElementsInArray (standardColours); i += 2)
...
void LookAndFeel::setColour (const int colourId, const Colour JUCE_ORIGINAL( & ) colour) noexcept
{
    const int index = colourIds.indexOf (colourId);
...
void LookAndFeel::drawSpinningWaitAnimation (Graphics& g, const Colour JUCE_ORIGINAL( & ) colour, int x, int y, int w, int h)
{
...
void LookAndFeel::drawRotarySlider (Graphics& g,
                                    ...
                                    Slider& slider)
{
#if defined( LE_PATCHED_JUCE ) && !defined( _DEBUG )
    LE_PATCH_UNREACHABLE_CODE
#else
...
public:
    ...
    void mouseWheelMove (const MouseEvent&, const MouseWheelDetails&) LE_PATCH( noexcept ) {}
};
...
public:
    //==============================================================================
    GlassWindowButton (const String& name, const Colour JUCE_ORIGINAL( & ) col,
...
{
#ifdef LE_PATCHED_JUCE
    jassert( !dynamic_cast<Component*> (&dcc) );
    Component* const fileListComp( nullptr );
#else
    ...
    LE_PATCH(      const int x = isDirectory ? 16 : 4 );
    JUCE_ORIGINAL( const int x = 32 );
    g.setColour (Colours::black);
    ...
    else
    {
        ...
        if ( isDirectory )
        {
            g.drawImageWithin (getDefaultFolderImage(), 2, 2, x - 4, height - 4,
                RectanglePlacement::centred | RectanglePlacement::onlyReduceInSize,
                false);
        }
    #else
...
JUCE_ORIGINAL( const Drawable* ) LE_PATCH( Image ) LookAndFeel::getDefaultFolderImage()
{
#ifdef LE_PATCHED_JUCE
    ...
    return ImageFileFormat::loadFrom( foldericon_png, sizeof ( foldericon_png ) );
#else
...
void LookAndFeel::drawBevel (Graphics& g, const int x, const int y, const int width, const int height,
                             const int bevelThickness, const Colour JUCE_ORIGINAL( & ) topLeftColour, const Colour JUCE_ORIGINAL( & ) bottomRightColour,
...
void LookAndFeel::drawGlassPointer (Graphics& g,
                                    const float x, const float y,
                                    const float diameter,
                                    const Colour JUCE_ORIGINAL( & ) colour, const float outlineThickness,
