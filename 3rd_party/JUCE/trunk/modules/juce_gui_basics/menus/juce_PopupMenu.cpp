...
public:
    ...
    //==============================================================================
    void mouseMove (const MouseEvent&) LE_PATCH( noexcept ) override    { timerCallback(); }
    ...
    void mouseWheelMove (const MouseEvent&, const MouseWheelDetails& wheel) LE_PATCH( noexcept ) override
    {
...
private:
        ...
        if (globalMousePos != lastMousePos || timeNow > lastMouseMoveTime + 350)
        {
            ...
            if (! isMovingTowardsMenu)
            {
                ...
                if (itemUnderMouse == nullptr && c != nullptr)
                    LE_PATCH( { jassert( !c->findParentComponentOfClass<PopupMenu::ItemComponent>() ); } )
                    JUCE_ORIGINAL( itemUnderMouse = c->findParentComponentOfClass<PopupMenu::ItemComponent>(); )
    ...
    bool canScroll() const noexcept                 { LE_PATCH_ASSUME(! ( childYOffset != 0 || needsToScroll )); return LE_PATCH( false ) JUCE_ORIGINAL( childYOffset != 0 || needsToScroll ); }
    bool isTopScrollZoneActive() const noexcept     { return LE_PATCH( false ) JUCE_ORIGINAL( canScroll() && childYOffset > 0 ); }
    bool isBottomScrollZoneActive() const noexcept  { return LE_PATCH( false ) JUCE_ORIGINAL( canScroll() && childYOffset < contentHeight - windowPos.getHeight() ); }
...
bool PopupMenu::MenuItemIterator::next()
{
    ...
    JUCE_ORIGINAL( commandManager  = item->commandManager; )
...
void PopupMenu::MenuItemIterator::addItemTo (PopupMenu& targetMenu)
{
    targetMenu.items.add (new Item (itemId, itemName, isEnabled, isTicked, customImage,
                                    ...
                                    nullptr,
                                    subMenu, LE_PATCH( nullptr ) JUCE_ORIGINAL( commandManager )));
}
