...
public:
    /** Destructor. */
    JUCE_ORIGINAL( virtual ~Drawable(); )
    ...
    */
    void setOriginWithOriginalSize (Point<float> LE_PATCH( const & ) originWithinParent);
    ...
    */
    JUCE_ORIGINAL( static Drawable* createFromSVG (const XmlElement& svgDocument); )

    /** Parses an SVG path string and returns it. */
    JUCE_ORIGINAL( static Path parseSVGPath (const String& svgPath); )
    ...
    */
    virtual Rectangle<float> getDrawableBounds() const LE_PATCH( noexcept ) = 0;
