...
ImagePixelData::~ImagePixelData() LE_PATCH( noexcept )
{
}
...
//==============================================================================
ImageType::ImageType() {}
ImageType::~ImageType() LE_PATCH( noexcept ) {}
...
SoftwareImageType::~SoftwareImageType() LE_PATCH( noexcept ) {}
...
int SoftwareImageType::getTypeID() const LE_PATCH( noexcept )
{
    return 2;
}
...
NativeImageType::~NativeImageType() LE_PATCH( noexcept ) {}

int NativeImageType::getTypeID() const LE_PATCH( noexcept )
{
    return 1;
}
...
public:
    ...
    void initialiseBitmapData (Image::BitmapData& bitmap, int x, int y, Image::BitmapData::ReadWriteMode mode) LE_PATCH( noexcept ) override
    {
...
Image::Image (const Image& other) LE_PATCH( noexcept )
    : image (other.image)
{
}

Image& Image::operator= (const Image& other) LE_PATCH( noexcept )
{
    image = other.image;
    return *this;
}
...
Image::PixelFormat Image::getFormat() const noexcept    { LE_PATCH_ASSUME( image.get() ); LE_PATCH_WINDOWS( LE_PATCH_ASSUME( image->pixelFormat != SingleChannel ); ) return image == nullptr ? UnknownFormat : image->pixelFormat; }
...
Image Image::convertedToFormat (PixelFormat newFormat) const
{
    ...
    {
        LE_PATCH_WINDOWS( LE_PATCH_UNREACHABLE_CODE )
        if (! hasAlphaChannel())
        {
...
Colour Image::BitmapData::getPixelColour (const int x, const int y) const noexcept
{
    ...
    switch (pixelFormat)
    {
        ...
        case Image::SingleChannel: LE_PATCH_WINDOWS( LE_PATCH_UNREACHABLE_CODE ) return Colour (((const PixelAlpha*) pixel)->getUnpremultipliedARGB());
        default: LE_PATCH_UNREACHABLE_CODE                   jassertfalse; break;
    }
...
void Image::BitmapData::setPixelColour (const int x, const int y, Colour colour) const noexcept
{
    ...
    switch (pixelFormat)
    {
        ...
        case Image::SingleChannel: LE_PATCH_WINDOWS( LE_PATCH_UNREACHABLE_CODE )  ((PixelAlpha*) pixel)->set (col); break;
...
static void performPixelOp (const Image::BitmapData& data, const PixelOperation& pixelOp)
{
    switch (data.pixelFormat)
    {
        ...
        case Image::SingleChannel: LE_PATCH_WINDOWS( LE_PATCH_UNREACHABLE_CODE ) PixelIterator<PixelAlpha>::iterate (data, pixelOp); break;
        default:                    jassertfalse; break;
    }
}
