...
LE_PATCH( LE_WEAK_SYMBOL ) PNGImageFormat png;
#else
...
ImageFileFormat* ImageFileFormat::findImageFormatForStream (InputStream& input) LE_PATCH( noexcept )
{
#ifdef LE_PATCHED_JUCE
    #ifndef NDEBUG
        const int64 streamPos = input.getPosition();
        jassert( png.canUnderstand (input) && input.setPosition (streamPos) );
    #endif // NDEBUG
    return &png;
#else
    const int64 streamPos = input.getPosition();
...
{
#ifdef LE_PATCHED_JUCE
    jassert( png.usesFileExtension( file ) );
    return &png;
#else
    for (ImageFileFormat** i = DefaultImageFormats::get(); *i != nullptr; ++i)
