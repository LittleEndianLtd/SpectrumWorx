...
namespace GradientPixelIterators
{
    ...
    public:
        ...
        #if !( defined( LE_PATCHED_JUCE ) && defined( _MSC_VER ) && defined( _DEBUG ) && !defined( _WIN64 ) )
...
namespace EdgeTableFillers
{
                                 ...
                                 const int alpha, const AffineTransform& transform, Graphics::ResamplingQuality quality, bool tiledFill)
    {
        ...
        case Image::ARGB:
            ...
            case Image::RGB:
                ...
                break;
            default: LE_PATCH_UNREACHABLE_CODE
                         ...
                         const PixelARGB* const lookupTable, const int numLookupEntries, const bool isIdentity, DestPixelType*)
    {
        if (g.isRadial)
        {
        #if defined( LE_PATCHED_JUCE ) && 0
            //...mrmlj...AlertWindow button...
...
struct ClipRegions
{
    class LE_PATCH_NOVTABLE Base  : public SingleThreadedReferenceCountedObject
    {
    public:
        Base() {}
        LE_PATCH( protected: ) JUCE_ORIGINAL( virtual ) ~Base() LE_PATCH( noexcept ) {} LE_PATCH( public: ) 

        typedef ReferenceCountedObjectPtr<Base> Ptr;

        virtual Ptr clone() const = 0;
        virtual Ptr applyClipTo (const Ptr& target) const LE_PATCH( noexcept ) = 0;

        virtual Ptr clipToRectangle (const Rectangle<int>&) LE_PATCH( noexcept ) = 0;
        virtual Ptr clipToRectangleList (const RectangleList<int>&) LE_PATCH( noexcept ) = 0;
        virtual Ptr excludeClipRectangle (const Rectangle<int>&) LE_PATCH( noexcept ) = 0;
        ...
        virtual void translate (Point<int> LE_PATCH( const & ) delta) LE_PATCH( noexcept ) = 0;

        virtual bool clipRegionIntersects (const Rectangle<int>&) const LE_PATCH( noexcept ) = 0;
        virtual Rectangle<int> getClipBounds() const LE_PATCH( noexcept ) = 0;

        virtual void fillRectWithColour (SavedStateType&, const Rectangle<int>&, const PixelARGB colour, bool replaceContents) const LE_PATCH( noexcept ) = 0;
        virtual void fillRectWithColour (SavedStateType&, const Rectangle<float>&, const PixelARGB colour) const LE_PATCH( noexcept ) = 0;
        virtual void fillAllWithColour (SavedStateType&, const PixelARGB colour, bool replaceContents) const LE_PATCH( noexcept ) = 0;
        virtual void fillAllWithGradient (SavedStateType&, ColourGradient&, const AffineTransform&, bool isIdentity) const LE_PATCH( noexcept ) = 0;
        virtual void renderImageTransformed (SavedStateType&, const Image&, const int alpha, const AffineTransform&, Graphics::ResamplingQuality, bool tiledFill) const LE_PATCH( noexcept ) = 0;
        virtual void renderImageUntransformed (SavedStateType&, const Image&, const int alpha, int x, int y, bool tiledFill) const LE_PATCH( noexcept ) = 0;
    };
    ...
    public:
        ...
        Ptr applyClipTo (const Ptr& target) const LE_PATCH( noexcept )   { return target->clipToEdgeTable (edgeTable); }

        Ptr clipToRectangle (const Rectangle<int>& r) LE_PATCH( noexcept )
        {
            edgeTable.clipToRectangle (r);
            return edgeTable.isEmpty() ? nullptr : this;
        }

        Ptr clipToRectangleList (const RectangleList<int>& r) LE_PATCH( noexcept )
        {
        ...
        Ptr excludeClipRectangle (const Rectangle<int>& r) LE_PATCH( noexcept )
        {
        ...
        void translate (Point<int> LE_PATCH( const & ) delta) LE_PATCH( noexcept )
        {
            edgeTable.translate ((float) delta.x, delta.y);
        }

        bool clipRegionIntersects (const Rectangle<int>& r) const LE_PATCH( noexcept )
        {
            return edgeTable.getMaximumBounds().intersects (r);
        }

        Rectangle<int> getClipBounds() const LE_PATCH( noexcept )
        {
            return edgeTable.getMaximumBounds();
        }

        void fillRectWithColour (SavedStateType& state, const Rectangle<int>& area, const PixelARGB colour, bool replaceContents) const LE_PATCH( noexcept )
        {
        ...
        void fillRectWithColour (SavedStateType& state, const Rectangle<float>& area, const PixelARGB colour) const LE_PATCH( noexcept )
        {
        ...
        void fillAllWithColour (SavedStateType& state, const PixelARGB colour, bool replaceContents) const LE_PATCH( noexcept )
        {
            state.fillWithSolidColour (edgeTable, colour, replaceContents);
        }

        void fillAllWithGradient (SavedStateType& state, ColourGradient& gradient, const AffineTransform& transform, bool isIdentity) const LE_PATCH( noexcept )
        {
            state.fillWithGradient (edgeTable, gradient, transform, isIdentity);
        }

        void renderImageTransformed (SavedStateType& state, const Image& src, const int alpha, const AffineTransform& transform, Graphics::ResamplingQuality quality, bool tiledFill) const LE_PATCH( noexcept )
        {
            state.renderImageTransformed (edgeTable, src, alpha, transform, quality, tiledFill);
        }

        void renderImageUntransformed (SavedStateType& state, const Image& src, const int alpha, int x, int y, bool tiledFill) const LE_PATCH( noexcept )
        {
    ...
    public:
        ...
        Ptr applyClipTo (const Ptr& target) const LE_PATCH( noexcept )   { return target->clipToRectangleList (clip); }
        ...
        bool clipRegionIntersects (const Rectangle<int>& r) const LE_PATCH( noexcept )   { return clip.intersects (r); }
        Rectangle<int> getClipBounds() const LE_PATCH( noexcept )                        { return clip.getBounds(); }
...
public:
    ...
    void fillWithSolidColour (IteratorType& iter, const PixelARGB colour, bool replaceContents) const
    {
        ...
        switch (destData.pixelFormat)
        {
            ...
            default: LE_PATCH_UNREACHABLE_CODE           EdgeTableFillers::renderSolidFill (iter, destData, colour, replaceContents, (PixelAlpha*) 0); break;
        }
    }
    ...
    void fillWithGradient (IteratorType& iter, ColourGradient& gradient, const AffineTransform& trans, bool isIdentity) const
    {
        ...
        switch (destData.pixelFormat)
        {
            ...
            default: LE_PATCH_UNREACHABLE_CODE           EdgeTableFillers::renderGradient (iter, destData, gradient, trans, lookupTable, numLookupEntries, isIdentity, (PixelAlpha*) 0); break;
        }
    }
...
public:
    ...
    void setOrigin (int x, int y) LE_PATCH( noexcept ) override                                       { stack->transform.setOrigin (Point<int> (x, y)); }
    void addTransform (const AffineTransform& t) LE_PATCH( noexcept ) override                        { stack->transform.addTransform (t); }
    float getPhysicalPixelScaleFactor() LE_PATCH( noexcept ) override                                 { return stack->transform.getPhysicalPixelScaleFactor(); }
    Rectangle<int> getClipBounds() const LE_PATCH( noexcept ) override                                { return stack->getClipBounds(); }
    bool isClipEmpty() const LE_PATCH( noexcept ) override                                            { return stack->clip == nullptr; }
    bool clipRegionIntersects (const Rectangle<int>& r) LE_PATCH( noexcept ) override                 { return stack->clipRegionIntersects (r); }
    bool clipToRectangle (const Rectangle<int>& r) LE_PATCH( noexcept ) override                      { return stack->clipToRectangle (r); }
    bool clipToRectangleList (const RectangleList<int>& r) LE_PATCH( noexcept ) override              { return stack->clipToRectangleList (r); }
    void excludeClipRectangle (const Rectangle<int>& r) LE_PATCH( noexcept ) override                 { stack->excludeClipRectangle (r); }
    ...
    void restoreState() LE_PATCH( noexcept ) override                                                 { stack.restore(); }
    ...
    void setFill (const FillType& fillType) LE_PATCH( noexcept ) override                             { stack->setFillType (fillType); }
    void setOpacity (float newOpacity) LE_PATCH( noexcept ) override                                  { stack->fillType.setOpacity (newOpacity); }
    void setInterpolationQuality (Graphics::ResamplingQuality quality) LE_PATCH( noexcept ) override  { stack->interpolationQuality = quality; }
    ...
    void setFont (const Font& newFont) LE_PATCH( noexcept ) override                                  { stack->font = newFont; }
    const Font& getFont() LE_PATCH( noexcept ) override                                               { return stack->font; }
