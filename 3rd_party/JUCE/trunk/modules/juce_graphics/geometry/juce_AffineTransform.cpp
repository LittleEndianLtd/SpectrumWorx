...
{
    JUCE_ORIGINAL( double ) LE_PATCH( float ) determinant = (mat00 * mat11 - mat10 * mat01);

    if (determinant != JUCE_ORIGINAL( 0.0 ) LE_PATCH( 0.0f ) )
    {
        determinant = JUCE_ORIGINAL( 1.0 ) LE_PATCH( 1.0f ) / determinant;
...
bool AffineTransform::isOnlyTranslation() const noexcept
{
    ...
    bool const result
    (
        ( reinterpret_cast<unsigned int const &>( mat01 ) == 0x00000000 ) &&
        ( reinterpret_cast<unsigned int const &>( mat10 ) == 0x00000000 ) &&
        ( reinterpret_cast<unsigned int const &>( mat00 ) == 0x3F800000 ) &&
        ( reinterpret_cast<unsigned int const &>( mat11 ) == 0x3F800000 )
    );
    return result;
#else
    return (mat01 == 0)
