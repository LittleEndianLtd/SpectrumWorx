...
namespace PathHelpers
{
    ...
    inline JUCE_ORIGINAL( double ) LE_PATCH( float ) lengthOf (float x1, float y1, float x2, float y2) noexcept
    {
        return juce_hypot (JUCE_ORIGINAL( (double) ) (x1 - x2), JUCE_ORIGINAL( (double) ) (y1 - y2));
    }
}
...
Path Path::createPathWithRoundedCorners (const float cornerRadius) const
{
    ...
    while (n < numElements)
    {
        ...
        else if (type == lineMarker || type == closeSubPathMarker)
        {
            ...
            if (lastWasLine)
            {
                const JUCE_ORIGINAL( double ) LE_PATCH( float ) len1 = PathHelpers::lengthOf (startX, startY, joinX, joinY);

                if (len1 > 0)
                {
                    const JUCE_ORIGINAL( double ) LE_PATCH( float ) propNeeded = jmin (0.5, cornerRadius / len1);
                ...
                const JUCE_ORIGINAL( double ) LE_PATCH( float ) len2 = PathHelpers::lengthOf (endX, endY, joinX, joinY);

                if (len2 > 0)
                {
                    const JUCE_ORIGINAL( double ) LE_PATCH( float ) propNeeded = jmin (0.5, cornerRadius / len2);
