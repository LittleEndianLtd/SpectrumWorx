...
class Timer::TimerThread  : private Thread,
                            JUCE_ORIGINAL( private DeletedAtShutdown, )
...
public:
    ...
    TimerThread()
    #if defined( LE_PATCHED_JUCE ) && defined( NDEBUG )
        : Thread (String::empty),
    #else
        : Thread ("Juce Timer"),
...
#ifdef LE_PATCHED_JUCE
union TimerThreadSingletonStorage
{
    double aligner;
    char   storage[ sizeof( Timer::TimerThread ) ];
};
static TimerThreadSingletonStorage timerThreadSingletonStorage;
void Timer::TimerThread::createSingleton()
{
    LE_PATCH_ASSUME( instance == nullptr );
    instance = new ( & timerThreadSingletonStorage ) TimerThread;
    LE_PATCH_ASSUME( instance == reinterpret_cast<TimerThread const *>( &timerThreadSingletonStorage ) );
}
void Timer::TimerThread::destroySingleton()
{
    LE_PATCH_ASSUME( instance == reinterpret_cast<TimerThread const *>( &timerThreadSingletonStorage ) );
    TimerThread::instance->~TimerThread();
    TimerThread::instance = nullptr;
}
void Timer::destroyThread()
{
    if ( TimerThread::instance )
        TimerThread::destroySingleton();
}
#endif // LE_PATCHED_JUCE
...
#if JUCE_DEBUG
#ifdef LE_PATCHED_JUCE
    SortedSet <Timer*> & activeTimers() { static SortedSet <Timer*> activeTimers_; return activeTimers_; }
    Timer::TimerThread::LockType & activeTimersLock() { static Timer::TimerThread::LockType lock; return lock; }
#else
...
{
   #if JUCE_DEBUG
    const TimerThread::LockType::ScopedLockType sl ( LE_PATCH( activeTimersLock() ) JUCE_ORIGINAL( TimerThread::lock ));
    activeTimers LE_PATCH( () ).add (this);
   #endif
}
...
Timer::~Timer()
{
   ...
   #if JUCE_DEBUG
    activeTimers LE_PATCH( () ).removeValue (this);
   #endif
}
...
void Timer::startTimer (const int interval) noexcept
{
    ...
    jassert (activeTimers LE_PATCH( () ).contains (this));
   #endif
...
{
    JUCE_ORIGINAL( const TimerThread::LockType::ScopedLockType sl (TimerThread::lock); )
    ...
    #ifdef LE_PATCHED_JUCE
        if ( TimerThread::instance )
        {//...mrmlj...static (de)initialization order fiasco...
            TimerThread::LockType::ScopedLockType const sl( TimerThread::lock );
            TimerThread::instance->removeTimer( this );
        }
    #else
